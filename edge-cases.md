...
This document outlines the four critical edge cases for the Invoice Automation system and the proposed solutions.

## 1. The Timezone Trap
**The Problem:** Your cron job is scheduled to run at `0 9 * * *` (9:00 AM). However, this runs at 9:00 AM **Server Time**. If you deploy this website to the cloud (like Render, Heroku, or AWS), cloud servers run on UTC Time by default. 9:00 AM UTC is 2:30 PM IST.
...
**The Solution:** We can add a simple 2-second delay between each webhook call inside your Express `for` loop so the emails trickle out naturally and mimic human sending behavior.

## 3. Invalid Client Email Addresses
**The Problem:** If you accidentally type a client's email wrong (e.g., `john@gmail..com`), the Express server will push it to n8n, n8n will try to email it, and Gmail will reject it. Because it fails, Express puts it in the Queue and tries 10 more times, failing every time until it gives up. Currently, you have no way of knowing *why* it gave up unless you manually inspect the database.
